...
/**
 * @file plugins/themes/AmcadX/AmcadXThemePlugin.inc.php
 *
 * Tema AMCAD X para OJS
 *
 * @class AmcadXThemePlugin
 * @ingroup plugins_themes_AmcadX
 *
 * @brief AMCAD X theme
 */

import('lib.pkp.classes.plugins.ThemePlugin');
class AmcadXThemePlugin extends ThemePlugin {
	...
	public function init() {
		...
		$this->addOption('baseColour', 'colour', array(
			'label' => 'plugins.themes.AmcadX.option.colour.label',
			'description' => 'plugins.themes.AmcadX.option.colour.description',
			'default' => '#1e3a8a',
		));
		...
		if (!preg_match('/^#[0-9a-fA-F]{1,6}$/', (string) $baseColour)) $baseColour = '#1e3a8a'; // pkp/pkp-lib#11974
		if ($baseColour !== '#1e3a8a') {
	...
	public function getDisplayName() {
			return __('plugins.themes.AmcadX.name');
	}
	...
	public function getDescription() {
			return __('plugins.themes.AmcadX.description');
	}
